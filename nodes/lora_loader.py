...
class TurboWanLoRALoader:
    ...
    * **Kohya** — keys end in ``.lora_down.weight`` / ``.lora_up.weight``
    ...
    def load_lora(
        ...
        # weights_only=False is required for LoRA files that store non-tensor
        # metadata (e.g. config dicts).  Only load LoRA files from trusted
        # sources – arbitrary pickle execution is possible otherwise.
        logger.log(f"Loading LoRA checkpoint...")
        lora_state_dict = torch.load(
            str(lora_path), map_location="cpu", weights_only=False
        )
